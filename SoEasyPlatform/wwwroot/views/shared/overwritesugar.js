...
var SugarContext = {
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                if (value.maxHeight != null) {
                    ...
                    $(element).parent().next().find(".comboTreeItemTitle ").first().next()
                        .css("height", value.maxHeight + 20);
                }
    ...
    Open: function(divElement, data) {
        ...
        //解决方案页 下拉框高度百分比
        if ($("div[name=solutiondiv]").length > 0) {
            var layercontentdiv = $("div[name=solutiondiv]").parent();
            layercontentdiv.css("cssText","height:80% !important");
            $("div.layui-layer-btn.layui-layer-btn-").css({ 'padding-top': 0});  
        }
